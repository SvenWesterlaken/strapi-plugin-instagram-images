...
exports.default = ({ strapi }) => ({
    async getShortLivedToken(redirect_uri, code, state) {
        ...
        const apiResult = await fetchInstagram_1.default.callInstagramApi("/oauth/access_token", {
    ...
    async getLongLivedToken() {
        ...
        if (settings.shortLivedAccessToken === undefined) {
            return {
                error: "Instagram long lived token error, there is no short lived token!",
                status: 400,
            };
        }
        const apiResult = await fetchInstagram_1.default.callInstagramGraph("/access_token", {
        ...
        if (apiResult.access_token !== undefined) {
            ...
            settings.expiresAt = new Date(new Date().getTime() + apiResult.expires_in * 1000);
        }
        await setPluginSettings(settings);
        return apiResult;
    },
    async refreshLongLivedToken() {
        ...
        const apiResult = await fetchInstagram_1.default.callInstagramGraph("/refresh_access_token", {
    ...
    async checkTokenExpiration() {
        ...
        const diffExpiration = dateUtils_1.default.dateDifferenceToNow(settings.expiresAt);
        const diffRefreshed = dateUtils_1.default.dateDifferenceToNow(settings.refreshTime);
